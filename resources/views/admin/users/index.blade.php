...
<!--breadcrumbs-->
  <div id="content-header">
    <div id="breadcrumb"> <a title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Quản lý nhân viên </a></div>
  </div>
<!--End-breadcrumbs-->
                            ...
                            <table id="zero_config" class="table table-striped table-bordered">
                                ...
                                @foreach($staffs as $staff)
                                    <tr>
                                        ...
                                        <td  style="text-align: center;">
                                        <?php 
                                            $temp = get_all_permission($staff->id);
                                        ?>
                                        @if($temp)
                                            @foreach($temp as $permission)
                                            <span class="date badge badge-warning">{{$permission}}</span> 
                                            @endforeach
                                        @endif
                                        </td>
                                        <td  style="text-align: center;">
                                        @if(auth()->user()->can('management') || auth()->user()->id == $staff->id)
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu">
                                                ...
                                                <li><a class="delete-question" href="{{route('add-permission', ['user' => $staff->id])}}">Thêm quyền</a></li>
                            ...
                            <table id="zero_config" class="table table-striped table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Vai trò</th>
                                        <th>Quyền</th>
                                        <th>Chức năng cụ thể</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($roleT as $role => $permission)
                                    <tr>
                                        <td></td>
                                        <td>{{$role}}</td>
                                        <td>{{implode(",", $permission)}}</td>
                                        <td>
                                          @if($permission[0] == "management")
                                            - Cấp quản lý cao nhất, được quyền truy cập và thực hiện các thao tác trên mọi menu.<br/>
                                            - Được quyền tạo user admin và chỉnh sửa quyền của các admin đó.
                                          @elseif($permission[0] == "management_quarter")
                                            - Quyền quản lý quý. </br/>
                                            - Được truy cập vào tab "quản lý nhân viên", nhưng không được quyền thêm nhân viên khác.
                                          @elseif($permission[0] == "management_questions")  
                                            - Quyền quản lý câu hỏi. </br>
                                            - Được truy cập vào tab "quản lý nhân viên", nhưng không được quyền thêm nhân viên khác.
                                          @elseif($permission[0] == "management_agency")  
                                            - Quyền quản lý đại lý. </br>
                                            - Được truy cập vào tab "quản lý nhân viên", nhưng không được quyền thêm nhân viên khác.
                                          @elseif($permission[0] == "management_common")  
                                            - Quyền quản lý thông tin chung. </br>
                                            - Được truy cập vào tab "quản lý nhân viên", nhưng không được quyền thêm nhân viên khác.  
                                          @elseif($permission[0] == "management_gift")  
                                            - Quyền quản lý thông tin quà tặng. </br>
                                            - Không được quyền xem thông tin mã nạp. </br>
                                            - Được truy cập vào tab "quản lý nhân viên", nhưng không được quyền thêm nhân viên khác. 
                                          @elseif($permission[0] == "management_news")  
                                            - Quyền quản lý tin tức. </br>
                                            - Được truy cập vào tab "quản lý nhân viên", nhưng không được quyền thêm nhân viên khác. 
                                          @elseif($permission[0] == "management_rule")  
                                            - Quyền quản lý thể lệ. </br>
                                            - Được truy cập vào tab "quản lý nhân viên", nhưng không được quyền thêm nhân viên khác. 
                                          @elseif($permission[0] == "management_videos")  
                                            - Quyền quản lý video. </br>
                                            - Được truy cập vào tab "quản lý nhân viên", nhưng không được quyền thêm nhân viên khác. 
                                          @endif
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabelLL">Tạo mới nhân viên (Tự động active)</h5>
      </div>
